...
def scrape_player_data():
    with sync_playwright() as p:
        ...
        try:
            ...
            for i in range(option_count):
                ...
                while True:
                    ...
                    for j in range(total_rows):
                        ...
                        radio = row.locator('css=span.ysf-player-icon.ysf-player-icon-notes.yfa-icon.playernote-recent')
                        radio.scroll_into_view_if_needed()
                        radio.wait_for_element_state('visible')  # Ensure the element is visible
                        radio.click()

                        # Wait for the player card to load and extract news
                        page.wait_for_selector('#player-note-content', timeout=5000)  # Wait for the player card to appear
                        player_card = page.locator('#player-note-content')
                        player_news = player_card.locator("css=section.player-notes").text_content(timeout=5000)
                        print(player_news)
            ...
            output_file = "player_data_by_stat.xlsx"
